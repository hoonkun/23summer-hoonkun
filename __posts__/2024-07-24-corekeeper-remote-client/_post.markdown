...
Compose 는 UI 요소가 함수이며, 구성 가능 함수의 '구성' 자체는 비동기적으로 동시에 실행될 수 있지만 어쨌든 위에서 아래로 흐른다.  
...
Compose 는 상태도 함수로 만들어지며 `by` 를 사용한 Delegate 가 조금 난해한 편이긴 하지만 그걸 신경쓰지 않아도 될 정도로 잘 되어있다.
...
Compose 에서는 함수 자체가 UI를 표현하기 때문에 아무것도 리턴하지 않으며, 따라서 타입도 없다. 모든 UI 는 그저 '`@Composable` 어노테이션으로 표시되는 구성 가능한 함수' 일 뿐이다.
...
Compose 에서는 어떤 특정한 뷰에만 사용할 수 있는 수정 옵션은 모두 함수의 인수로 받는다. 예를 들어 Text 의 글씨 색을 수정하려고 한다면, 수정자를 붙히는게 아니라 Text 구성 가능 함수의 textStyle 인수(혹은 color 인수)에 값을 줌으로써 이루어진다.
